...
<?php 
	...
	printf("<h1 align='center'>%s - KEY</h1>", $ename);
	print("<h4>Name ________________________</h4>\n");
	printf("<h3>Directions: %s</h3>", $instructions);
	
	// retrieve list of questions
	$query = "SELECT answer FROM exam_question NATURAL JOIN question WHERE eid=$eid;";
	$result = $db->query($query);
	print("<ol>");
	while($row =  $result->fetch()){
		printf("<li><p>%s</p></li>", $row['answer']);
	}
